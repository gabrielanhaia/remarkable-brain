...
function seededRepo(): Repo {
  ...
  repo.upsertPage({
    ...
    extractedText: 'Ordio pricing decision',
    pageType: 'decision',
    openLoop: false,
  ...
  repo.upsertPage({
    ...
    extractedText: 'Follow up with Ordio team',
    pageType: 'meeting_notes',
    openLoop: true,
    openLoopDescription: 'follow up with Ordio',
  ...
  repo.upsertPage({
    ...
    extractedText: 'Ordio hidden secret',
    openLoop: true,
...
test('GET /api/pages/:id returns the full page + imageUrl', () => {
  ...
  expect(res.status).toBe(200);
  const body = res.body as any;
  expect(body.extractedText).toContain('Ordio');
  expect(body.imageUrl).toBe('/images/n1/page-1.png');
  expect(body.entities[0].name).toBe('Ordio');
});
...
test('GET /api/search returns hits for a query', () => {
  const res = call(routes, '/api/search', {}, 'q=Ordio');
  expect(res.status).toBe(200);
  const body = res.body as any[];
...
test('GET /api/search applies the open_loop filter', () => {
  const res = call(routes, '/api/search', {}, 'q=Ordio&open_loop=1');
  const body = res.body as any[];
  expect(body.map((h) => h.pageId)).toEqual(['p2']);
});

test('GET /api/search applies the type filter', () => {
  const res = call(routes, '/api/search', {}, 'q=Ordio&type=decision');
  const body = res.body as any[];
...
test('GET /api/entities lists entities with page counts', () => {
  ...
  expect(res.status).toBe(200);
  const body = res.body as any[];
  expect(body[0]).toMatchObject({ name: 'Ordio', type: 'company', pageCount: 2 });
});

test('GET /api/entities/:name/timeline is chronological', () => {
  const res = call(routes, '/api/entities/:name/timeline', { name: 'Ordio' });
  expect(res.status).toBe(200);
  const body = res.body as any[];
