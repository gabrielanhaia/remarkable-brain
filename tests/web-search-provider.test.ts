...
beforeEach(() => {
  ...
  repo.upsertPage({
    ...
    extractedText: 'Ordio in my personal notebook',
    pageType: 'note',
    openLoop: false,
  });
...
test('returns matching hits, ranked', () => {
  const hits = provider.search('Ordio');
...
test('does not throw on FTS operator characters', () => {
  expect(() => provider.search('Ordio: pricing*')).not.toThrow();
});

test('notebook filter restricts to one notebook by display name', () => {
  const hits = provider.search('Ordio', { notebook: 'Personal' });
  expect(hits.map((h) => h.pageId)).toEqual(['p3']);
});

test('type filter restricts to a page type', () => {
  const hits = provider.search('Ordio', { type: 'decision' });
  expect(hits.map((h) => h.pageId)).toEqual(['p1']);
});

test('openLoop filter keeps only open-loop pages', () => {
  const hits = provider.search('Ordio', { openLoop: true });
  expect(hits.map((h) => h.pageId)).toEqual(['p2']);
});

test('combined filters intersect', () => {
  expect(provider.search('Ordio', { notebook: 'Work Notes', type: 'decision' }).map((h) => h.pageId)).toEqual([
    'p1',
  ]);
  expect(provider.search('Ordio', { notebook: 'Work Notes', openLoop: true }).map((h) => h.pageId)).toEqual([
    'p2',
  ]);
});
