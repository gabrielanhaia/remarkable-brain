...
describe('config', () => {
  ...
  test('isUnderFolder is case-insensitive and recursive, files only', () => {
    expect(isUnderFolder('/Brain/Ordio', '/Brain')).toBe(true);
    expect(isUnderFolder('/brain/Ordio', 'Brain')).toBe(true); // case-insensitive
    expect(isUnderFolder('/Brain/sub/Deep', '/Brain')).toBe(true); // recursive
    expect(isUnderFolder('/Other/Ordio', '/Brain')).toBe(false);
