...
test('FTS row is populated by trigger on page insert', () => {
  ...
  db.prepare(
    "INSERT INTO pages (id, notebook_id, page_number, extracted_text) VALUES ('p1','n1',1,'hello ordio world')"
  ).run();
  const hit = db.prepare("SELECT rowid FROM pages_fts WHERE pages_fts MATCH 'ordio'").get();
...
beforeEach(() => {
  ...
  repo.upsertPage({ id: 'p1', notebookId: 'n1', pageNumber: 1, writtenAt: '2026-01-01', extractedText: 'Ordio pricing decision', pageType: 'decision', openLoop: false, imagePath: '/img/p1.png' });
  repo.upsertPage({ id: 'p2', notebookId: 'n1', pageNumber: 2, writtenAt: '2026-02-01', extractedText: 'Follow up with Ordio team', pageType: 'meeting_notes', openLoop: true, openLoopDescription: 'follow up with Ordio', imagePath: '/img/p2.png' });
  repo.linkEntities('p1', [{ name: 'Ordio', type: 'company' }]);
  repo.linkEntities('p2', [{ name: 'Ordio', type: 'company' }]);
});

test('searchNotes returns receipts', () => {
  const hits = repo.searchNotes('Ordio');
...
test('getEntityTimeline is chronological', () => {
  const t = repo.getEntityTimeline('Ordio');
...
test('purgeNotebook returns image paths and removes rows', () => {
  ...
  expect(repo.searchNotes('Ordio').length).toBe(0);
...
test('runSync indexes only #brain, honors hard-exclusion, skips unchanged', async () => {
  ...
  const extract = vi.fn().mockResolvedValue({
    extracted_text: 'hi', page_type: 'idea', entities: [{ name: 'Ordio', type: 'company' }], open_loop: false, open_loop_description: '',
  });
...
beforeEach(() => {
  ...
  repo.upsertPage({ id: 'p1', notebookId: 'n1', pageNumber: 1, writtenAt: '2026-01-01', extractedText: 'Ordio pricing', openLoop: true, openLoopDescription: 'decide price', imagePath: '/i/p1.png' });
  repo.linkEntities('p1', [{ name: 'Ordio', type: 'company' }]);
  handlers = buildToolHandlers(repo);
});

test('search_notes returns receipts', () => {
  const r = handlers.search_notes({ query: 'Ordio' });
...
test('get_entity_timeline works', () => {
  expect(handlers.get_entity_timeline({ entity_name: 'Ordio' }).results.length).toBe(1);
});
test('get_page returns image path + text', () => {
  const r = handlers.get_page({ page_id: 'p1' });
  expect(r.imagePath).toBe('/i/p1.png');
  expect(r.extractedText).toContain('Ordio');
});
```
